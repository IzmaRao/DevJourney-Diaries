    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                ...
                const response = await fetch(`https://dev-journey-diaries.vercel.app/api/blog/${unwrappedParams.id}`, { cache: 'no-store' });
                if (!response.ok) {
            ...
            {blog && (
                <div className='featured-detailblog' key={blog.id}>
                    ...
                    <div className='featured-detailblog-content space-y-6'>
                        <h1 className='featured-detailblog-h text-[#0d0d0d]'>{blog.blogname}</h1>
